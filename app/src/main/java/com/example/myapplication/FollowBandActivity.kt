...
class FollowBandActivity : AppCompatActivity() {
    ...
    private fun init() {
        ...
        myAdapter= CustomAdapterFollowBand(newArray){ evento ->
    ...
    private fun getDataFollow() {
        ...
        dbreferes.orderByChild("emailBand").equalTo(auth.currentUser?.email).addValueEventListener(object :
            ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                Log.d("DataCheck", "newArray size: ${newArray.size}")
                Log.d("DataCheck", "newArray size: ${auth.currentUser?.email}")
                newArray.clear()
                if(snapshot.exists()){
            ...
            override fun onCancelled(error: DatabaseError) {
                ...
                Toast.makeText(this@FollowBandActivity, "Error en la lectura de datos: ${error.message}", Toast.LENGTH_SHORT).show()
            }
